...
public class ExclusaoJaponesApiSyncService : SyncApiServiceBase<ExclusaoJapones> {
    private readonly ILogger _logger;
    private TextoJaponesApiService? _api;

    public ExclusaoJaponesApiSyncService(Connection connection, ILogger logger) : base(connection) {
        _logger = logger;
    }
    ...
    public override async Task GetAsync(DateTime since, ProgressCallback<ExclusaoJapones> onPageReceived) {
        _logger.Information("Iniciando busca de atualizações de ExclusaoJapones para a conexão {Description}", Connection.Description);
        ...
        do {
            ...
            if (pagedResponse?.Content == null || !pagedResponse.Content.Any()) {
                ...
                var entities = Mapper.Map<List<ExclusaoJapones>>(pagedResponse.Content);
    ...
    public override async Task SaveAsync(List<ExclusaoJapones> entities, string extra) {
        var dtos = Mapper.Map<List<ExclusaoDto>>(entities);
        await _api!.SendAsync(dtos);
    }

    public override async Task DeleteAsync(List<ExclusaoJapones> entities, string extra) {
        var dtos = Mapper.Map<List<ExclusaoDto>>(entities);
